...
int main() {
    ...
    while (input2 >> inputLine) {
        strings[var++] = inputLine;
    }

    for (var = 0; var < 250; var++) {
        for (var2 = var + 1; var2 < 250; var2++) {
            std::string string1 = strings[var];
            const char *string2 = strings[var2].c_str();
            char val1, val2;
            uint8_t amountOfDifferences{0};
            ...
            if (amountOfDifferences == 1) {
                std::cout << "Solution part 2: " << output.str();
            }
        }
    }
}